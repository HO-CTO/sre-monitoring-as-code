...
local dashboardDocs = {
  ...
  productView: '#### Product View\n\n**Purpose:** Observability of all the user journeys running on an individual product.\n\n**Directions for use:** Distinct user journeys are presented as row panels. SLIs are presented as stat panels indicating the SLO Status, Error Budget and SLO target as set in the mixing defs. Colour thresholds have been set to highlight and identify issues. SLI Types (such as Availability and Latency) are aggregated into a single SLI Stat Panel. The dashboard can be filtered by Environment.',
