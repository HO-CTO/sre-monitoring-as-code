...
// success - Metric representing the total number of successes
      ...
      expr: |||
        sum without (%(selectorLabels)s) (label_replace(label_replace(
          (
            ...
            /
            (
            sum by(%(selectorLabels)s) (rate(%(successMetric)s{%(selectors)s}[%(evalInterval)s]))
            +
            sum by(%(selectorLabels)s) (rate(%(failureMetric)s{%(selectors)s}[%(evalInterval)s]))
            )
          ),
...
local createGraphPanel(sliSpec) =
  ...
  graphPanel.new(
    ...
    prometheus.target(
      |||
        sum(rate(%(successMetric)s{%(selectors)s}[%(evalInterval)s]) or vector(0)) + sum(rate(%(failureMetric)s{%(selectors)s}[%(evalInterval)s]) or vector(0))
      ||| % {
        failureMetric: targetMetrics.failure,
        successMetric: targetMetrics.success,
    ...
    prometheus.target(
      |||
        ...
        /
        (
        sum(rate(%(successMetric)s{%(selectors)s}[%(evalInterval)s]))
        +
        sum(rate(%(failureMetric)s{%(selectors)s}[%(evalInterval)s]))
        )
      ||| % {
