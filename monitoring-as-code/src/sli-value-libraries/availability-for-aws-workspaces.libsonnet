  ...
  [
    {
      ...
      expr: |||
        sum without (%(selectorLabels)s) (label_replace(label_replace(
          (
            (
            sum by(%(selectorLabels)s) (rate(%(unhealthyMetric)s{%(selectors)s}[%(evalInterval)s]))
            +
            sum by(%(selectorLabels)s) (rate(%(maintenanceMetric)s{%(selectors)s}[%(evalInterval)s]))
            )
            /
            (
            sum by(%(selectorLabels)s) (rate(%(availableMetric)s{%(selectors)s}[%(evalInterval)s]))
            +
            sum by(%(selectorLabels)s) (rate(%(unhealthyMetric)s{%(selectors)s}[%(evalInterval)s]))
            +
            sum by(%(selectorLabels)s) (rate(%(maintenanceMetric)s{%(selectors)s}[%(evalInterval)s]))
      ...
      ||| % {
        unhealthyMetric: targetMetrics.unhealthy,
        maintenanceMetric: targetMetrics.maintenance,
        availableMetric: targetMetrics.available,
...
local createGraphPanel(sliSpec) =
  ...
  graphPanel.new(
    ...
    prometheus.target(
      |||
        sum(rate(%(availableMetric)s{%(selectors)s}[%(evalInterval)s]) or vector(0))
        +
        sum(rate(%(unhealthyMetric)s{%(selectors)s}[%(evalInterval)s]) or vector(0))
        +
        sum(rate(%(maintenanceMetric)s{%(selectors)s}[%(evalInterval)s]) or vector(0))
      ||| % {
        unhealthyMetric: targetMetrics.unhealthy,
        maintenanceMetric: targetMetrics.maintenance,
        availableMetric: targetMetrics.available,
